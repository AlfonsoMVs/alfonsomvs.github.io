...
──────────Funciones PHP para los Addons que lo necesiten────────────────
\ ♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛*/
	ob_start();
	...
	$diamonds = $Functions->Get('seasonal_currency');
		...
		if(isset($_POST['changename'])){
			...
			if($user['cms_changename'] == 1){
				$_SESSION['ERROR_RETURN'] = "Lo sentimos, pero ya has cambiado tu nombre 1 vez";
					header("LOCATION: ". PATH ."/me?session");
			}elseif($online == 1){
				...
				$db->query("UPDATE rooms_data SET owner = '{$changename}' WHERE owner = '{$oldname}' LIMIT 1");
	...
	//PLACA DE REGALO
		if(isset($_POST['badgecode'])){
			...
			if($badgel->num_rows > 0){
				...
				$badgen = $db->query("SELECT * FROM users_badges WHERE badge_id = '{$cdbadge}' AND user_id = '{$userid}' LIMIT 1");
				if($badgen->num_rows > 0){
					...
					$_SESSION['GOOD_RETURN'] = "Has reclamado tu placa con &eacute;xito.";
						header("LOCATION: ". PATH ."/me?badgeaction");
					$do1 = $db->query("UPDATE users SET seasonal_currency = seasonal_currency - '{$cpbadge}' WHERE username = '{$_SESSION['username']}'");
					$dbBadge= array();
					$dbBadge['user_id'] = $userid;
					$dbBadge['badge_id'] = $cdbadge;
					$query = $db->insertInto('users_badges', $dbBadge);	
				}else{
		...
		if(isset($_POST['buscador'])){ 
			...
			if(empty($buscar)){
				...
				if ($row = mysql_fetch_array($result)){
						header("LOCATION: ". PATH ."/Perfil?p=".$row['id']."");
				}else{ 
				...
				if(!empty($totalRows)){
					while($row = $sqlBuscar->fetch_array()){
						...
						if($row['max_uses'] <= $row['cur_uses']){
							...
							$db->query("UPDATE items_vouchers SET cur_uses = cur_uses + '1'  WHERE voucher = '$code' LIMIT 1");
							$db->query("UPDATE users SET seasonal_currency = seasonal_currency + '".$row['value']."'  WHERE username = '{$_SESSION['username']}'");
							$db->query("INSERT INTO cms_vouchercontrol (user, voucher) VALUES ('{$_SESSION['username']}', '$code')");
			...
			if($badgel->num_rows > 0){
				...
				if($badgen->num_rows > 0){
					$_SESSION['ERROR_RETURN'] = "Al parecer ya posees esta placa.";
						header("LOCATION: ". PATH ."/Tienda=ComprarPlacas?badgeaction");
				}elseif($online == 1){
					$_SESSION['ERROR_RETURN'] = "Debes estar fuera del client para evitar errores";
						header("LOCATION: ". PATH ."/Tienda=ComprarPlacas?badgeaction");
				}elseif($diamonds >= $cpbadge){
	...
	//RARES
		...
		$rarel = $db->query("SELECT code,price,item_name FROM cms_rares WHERE id = '{$rarec}' LIMIT 1");
			if($rarel->num_rows > 0){
				...
				if($online == 1){
					$_SESSION['ERROR_RETURN'] = "Debes estar fuera del client para evitar errores";
						header("LOCATION: ". PATH ."/Tienda=ComprarRares?rareaction");
				}elseif($diamonds >= $cprare){
					$_SESSION['GOOD_RETURN'] = "Rare comprado con &eacute;xito";
						header("LOCATION: ". PATH ."/Tienda=ComprarRares?rareaction");		
					$do1 = $db->query("UPDATE users SET seasonal_currency = seasonal_currency - '{$cprare}' WHERE username = '{$_SESSION['username']}'");
					$dbrare= array();
					...
					$dbrare['group_id'] = 0;
					$dbrare['builders'] = 0;				
					$query = $db->insertInto('items_rooms', $dbrare);				
				}else{
	...
	//VIP
		if(isset($_POST['buyvip'])){
			if($user['rank'] >= 2){
				...
				$db->query("UPDATE users SET rank = '2', seasonal_currency = seasonal_currency - '25' WHERE username = '{$_SESSION['username']}'");
		...
		if(isset($_POST['BugReporter'])){
			...
			if($security->num_rows > 0){
				if($sec['time'] >= time() - 300){
					...
					$db->query("INSERT INTO cms_comments (username, comentario, type, time, posted_on, posted_in) VALUES ('". $_SESSION['username'] ."', '{$_POST['BugReporter']}', 'bug', '".time()."', '".date("Y-m-d ")."', '0')");
	...
	//FORGOT 
		if(isset($_POST['emailAddress'])){
			...
			if($buscar->num_rows > 0){
				...
				mail("$email",'Recuperación de contraseña - OneCMS by Jeihden', "Estimado usuario, hemos detectado la solicitud del cambio de tu contraseña correctamente. Si no es as&iacute; ignora este mensaje, de lo contrario haz clic <a href='". PATH ."/account/password/resetIdentity/9346b03cbb86c009501ce113cb38dce39ebba9c34a6416d8edef5ac544db7dddb5158bf4e86bc09c089a3ed7e87049f144888ef2b45e1a02986bdc8858d82ad0dd460/?c=$code'>Aqu&iacute;</a> o copia el siguiente enlace:<br> ". PATH ."/account/password/resetIdentity/9346b03cbb86c009501ce113cb38dce39ebba9c34a6416d8edef5ac544db7dddb5158bf4e86bc09c089a3ed7e87049f144888ef2b45e1a02986bdc8858d82ad0dd460/?c=$code <br> ¿No ha funcionado? ¡Manda nuevamente el formulario!");
				...
				$_SESSION['REG_ERROR'] = "&iexcl;El Email no se encuentra! Intenta registrandote <a href='/Registro'>Aqu&iacute;</a>";
					header("LOCATION: ". PATH ."#");
			}
		}
	//END FORGOT
