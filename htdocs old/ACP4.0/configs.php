...
ob_start();
	...
	if(isset($_POST['V3'])){
		...
		$action = $db->query("UPDATE cms_settings SET v3_style = '{$var}' WHERE id = '1' LIMIT 1");
		$db->query("INSERT INTO cms_stafflogs (username, action, message, rank, userid, timestamp) VALUES ('". $_SESSION['username'] ."','Configuraci&oacute;n General del Hotel', 'Ha Cambiado el Estilo a OneCMS V3', '". $user['rank'] ."', '". $user['id'] ."', '".date("Y-m-d ")."')");
		if($action){
			$_SESSION['GOOD_RETURN'] = "Estilo OneCMSV3 modificado Correctamente";
			header("LOCATION: ". HK ."/configs.php");
		}else{
			$_SESSION['ERROR_RETURN'] = "Ha ocurrido un error indeterminado";
			header("LOCATION: ". HK ."/configs.php");
					...
					<div class="panel">
						...
						<div class="panel-body">
							<form action="" method="post">
								...
								<div class="form-group">
									<label for="input-text" class="control-label">Estilo OneCMS V3</label>
									<?php if($data['v3_style'] == 1){ echo '<input type="submit" name="V3" class="btn btn-dark bg-red-300 color-white margin-left-10" style="float:right;"value="Desactivar" />';}else{ echo '<input type="submit" name="V3" class="btn btn-dark bg-blue-300 color-white margin-left-10" style="float:right;"value="Activar" />';} ?>
								</div>
								<div class="form-group">
									<label for="input-text" class="control-label">Diseño de la CMS</label>
									<br><select class="form-control" name="diseno">
										<option value="<?php echo $data['design']; ?>" selected>Diseño</option>
										<option value="normal">Normal</option>
										<option value="valentine">San Valent&iacute;n</option>
